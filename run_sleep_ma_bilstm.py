...
from src.sleepstaging.model_ma_bilstm import MA_CNN_BiLSTM
...
def plot_cm(cm, labels, outpath):
    ...
    im = ax.imshow(cm_norm, interpolation="nearest"); ax.set_title("Confusion Matrix (Row-normalized)")
...
def main():
    ...
    ckpt = MODELS / "best_ma_cnn_bilstm.pth"
    ...
    plot_cm(cm, CLASSES, RESULTS / "ma_bilstm_confusion_matrix.png")
    prec, rec, f1c, sup = precision_recall_fscore_support(y_true, y_pred, labels=list(range(len(CLASSES))), zero_division=0.0)
    with open(RESULTS / "ma_bilstm_per_class_metrics.csv", "w", newline="") as f:
    ...
    np.savetxt(RESULTS / "ma_bilstm_confusion_matrix_raw.csv", cm, fmt="%d", delimiter=",")
